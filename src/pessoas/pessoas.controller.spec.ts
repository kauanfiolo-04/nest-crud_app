...
describe('PessoasController', () => {
  ...
  it('create - deve usar o PessoasService com o argumento correto', async () => {
  ...
  it('findAll - deve usar o PessoasService', async () => {
    ...
    expect(pessoasServiceMock.findAll).toHaveBeenCalled();
    expect(result).toEqual(expected);
  });

  it('findOne - deve usar o PessoasService com o argumento correto', async () => {
    const argument = '1';
    const expected = { anyKey: 'anyValue' };
  ...
  it('update - deve usar o PessoasService com os argumentos corretos', async () => {
    const argument1 = '1';
    ...
    const result = await controller.update(argument1, argument2 as any, argument3 as any);

    expect(pessoasServiceMock.update).toHaveBeenCalledWith(+argument1, argument2, argument3);
    expect(result).toEqual(expected);
  });
  ...
  it('remove - deve usar o PessoaService com os argumentos corretos', async () => {
    ...
    expect(pessoasServiceMock.remove).toHaveBeenCalledWith(+argument1, argument2);
    expect(result).toEqual(expected);
  });
  ...
  it('uploadPicture - deve usar o PessoaService com os argumentos corretos', async () => {
    ...
    const result = await controller.uploadPicture(argument1 as any, argument2 as any);

    expect(pessoasServiceMock.uploadPicture).toHaveBeenCalledWith(argument1, argument2);
    expect(result).toEqual(expected);
  });
});
