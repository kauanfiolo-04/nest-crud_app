...
describe('Pessoas service', () => {
  ...
  describe('uploadPicture', () => {
    it('deve salvar a imagem corretamente e atualizar a pessoa', async () => {
      ...
      // Act
      const result = await pessoasService.uploadPicture(mockFile, tokenPayload as any);
    ...
    it('deve lancar BadRequestExcpetion se o arquivo for muito pequeno', async () => {
      // Arrange
      const mockFile = {
        originalname: 'test.png',
        size: 500,
      ...
      //Act & Assert
      await expect(pessoasService.uploadPicture(mockFile, tokenPayload as any)).rejects.toThrow(BadRequestException);
    });

    it('deve lancar NotFoundExcpetion se a pessoa nao for encontrada', async () => {
      // Arrange
      const mockFile = {
        originalname: 'test.png',
        size: 2000,
      ...
      // Act & Assert
      await expect(pessoasService.uploadPicture(mockFile, tokenPayload as any)).rejects.toThrow(NotFoundException);
    });
  });
});
